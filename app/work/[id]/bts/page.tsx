...
export default function BtsPage({ params }: { params: { id: string } }) {
  ...
  if (!project.bts || project.bts.length === 0) {
    return (
      <div className="bg-black min-h-screen flex items-center justify-center">
        <div className="text-center flex flex-col gap-4">
          <p className="font-condensed text-text-secondary text-sm tracking-label uppercase">No behind the scenes photos available</p>
          <Link
            href={`/work/${params.id}`}

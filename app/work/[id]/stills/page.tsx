...
export default function StillsPage({ params }: { params: { id: string } }) {
  ...
  if (!project.stills || project.stills.length === 0) {
    return (
      <div className="bg-black min-h-screen flex items-center justify-center">
        <div className="text-center flex flex-col gap-4">
          <p className="font-condensed text-text-secondary text-sm tracking-label uppercase">No stills available</p>
          <Link
            href={`/work/${params.id}`}
